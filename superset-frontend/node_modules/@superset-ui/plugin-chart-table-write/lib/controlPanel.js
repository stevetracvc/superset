...
const validateAggControlValues = (controls, values) => {
  const areControlsEmpty = values.every(val => (0, _core.ensureIsArray)(val).length === 0);
  return areControlsEmpty && isAggMode({
    controls
  }) ? [(0, _core.t)('Group By, Metrics or Percentage Metrics must have a value')] : [];
};
...
const queryMode = {
  ...
  default: null,
  options: [[_core.QueryMode.aggregate, _chartControls.QueryModeLabel[_core.QueryMode.aggregate]], [_core.QueryMode.raw, _chartControls.QueryModeLabel[_core.QueryMode.raw]]],
  mapStateToProps: ({
    controls
  }) => ({
...
const all_columns = {
  ...
  mapStateToProps: ({
    ...
    externalValidationErrors: isRawMode({
      controls
    }) && (0, _core.ensureIsArray)(controlState.value).length === 0 ? [(0, _core.t)('must have a value')] : []
  }),
...
const config = {
  controlPanelSections: [_chartControls.sections.legacyTimeseriesTime, {
    ...
    controlSetRows: [[{
      ...
      override: {
        ...
        mapStateToProps: (state, controlState) => {
          ...
          const originalMapStateToProps = _chartControls.sharedControls == null ? void 0 : (_sharedControls$group = _chartControls.sharedControls.groupby) == null ? void 0 : _sharedControls$group.mapStateToProps;
      ...
      name: 'all_columns',
      config: (0, _core.isFeatureEnabled)(_core.FeatureFlag.ENABLE_EXPLORE_DRAG_AND_DROP) ? dnd_all_columns : all_columns
    }], [{
      name: 'percent_metrics',
      config: { ...((0, _core.isFeatureEnabled)(_core.FeatureFlag.ENABLE_EXPLORE_DRAG_AND_DROP) ? dnd_percent_metrics : percent_metrics)
      }
    }], [{
    ...
    }], (0, _core.isFeatureEnabled)(_core.FeatureFlag.DASHBOARD_CROSS_FILTERS) || (0, _core.isFeatureEnabled)(_core.FeatureFlag.DASHBOARD_NATIVE_FILTERS) ? [{
      ...
      config: {
        ...
        default: false,
        description: (0, _core.t)('Show total aggregations of selected metrics. Note that row limit does not apply to the result.'),
        visibility: isAggMode
      }
    ...
    controlSetRows: [[{
      ...
      config: {
        ...
        renderTrigger: true,
        default: false,
        description: (0, _core.t)('Whether to align background charts with both positive and negative values at 0')
      }
    }, {
      name: 'color_pn',
      config: {
      ...
      config: {
        ...
        description: (0, _core.t)('Further customize how to display each column'),
        renderTrigger: true,
      ...
      config: {
        ...
        description: (0, _core.t)('Apply conditional color formatting to numeric columns'),
        ...
        mapStateToProps(explore, control, chart) {
          ...
          const numericColumns = Array.isArray(colnames) && Array.isArray(coltypes) ? colnames.filter((colname, index) => coltypes[index] === _core.GenericDataType.NUMERIC).map(colname => {
